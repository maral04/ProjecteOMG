...
PImage[] imgJugadorgir = new PImage[16];
...
void movEsquerre(PImage b) {
  ...
  if (posicio < 0) {
    ...
    //if (salta == false) {
      ...
      image(imgJugadorgir[tipusMoviment], posicio, posicioSalt);
      posicio = posicio - 3;
    //}
  }
}
...
void movimentPtg() {
  ...
  for (int i = liniaInici; i < liniafinal; i++) {
    ...
    //Guardar l'imatge girada en un nou array.
    //scale(-1,1);
    imgJugadorgir[i] = src[ptg].get(ptgx[i], ptgy[i], ptgw[i], ptgh[i]);
    //save("cross.png");
    //pushMatrix();
    //pushMatrix();
    //popMatrix();
